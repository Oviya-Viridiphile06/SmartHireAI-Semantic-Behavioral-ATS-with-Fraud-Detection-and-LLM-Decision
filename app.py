...
if st.button("Run ATS Evaluation"):
    ...
    for file in files:
        ...
        score, fraud, status, reason, exp = evaluate_candidate(
            text,
            jd,
            skills,
            file_path=file
        ...
        if "Missing:" in reason:
            llm_skills = reason.replace("Missing:", "").strip()

        # Decide display reason (VERY IMPORTANT FIX)
        ...
        if fraud:
            ...
            display_reason = "Insufficient skill match"

        elif status == "Shortlisted":
            display_reason = "Qualified"
